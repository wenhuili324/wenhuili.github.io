...
We applied machine learning techniques—specifically, dimensionality reduction methods such as Principal Component Analysis (PCA) and t-distributed Stochastic Neighbor Embedding (t-SNE)—to investigate the relationship between molecular orientation and shear rate in greater depth. Our analysis revealed that molecular orientation changes progressively with increasing shear rate at low to medium shear rates but becomes saturated at medium to high shear rates. Interestingly, fluid viscosity continues to decrease in this medium to high shear rates regime, which contradicts the assumptions of the power-law model.
